/**
 * EcoSimRPG - autorun.js (VERSION 4.0 - Synchronisation Majeure)
 *
 * DESCRIPTION :
 ...
 * Cette version vise à produire des mondes cohérents avec la logique de l'interface interactive.
 *
 ...
 * - Étape 4 (Simulation) : Intégration complète des règles de step4.js :
 * - Mortalité (vieillesse, accident, surpopulation).
 * - Succession dynastique (primogéniture, électif).
 * - Mariages stratégiques basés sur le statut.
 * - Croissance des stats/prestige avec malus d'âge/expérience.
 * - Gain passif pour les chômeurs.
 * - Promotions hiérarchiques et migrations.
 *
 * UTILISATION :
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    "Redpine", "Ironspire", "Gloomreach", "Cragwatch", "Violet Hollow"];
            ...
            // AMÉLIORÉ: Distribution des types de lieux alignée sur la logique de `step1.js`
            const typesToCreate = [];
            ...
            // NOUVEAU: Ajout de routes garanties pour la capitale, comme dans `step1.js`
        ...
        async _step2_buildEconomy() {
            ...
            // AMÉLIORÉ: Logique de résolution de dépendances alignée sur `step2.js`
            let attempts = 0;
        ...
        async _step3_generatePopulation() {
            ...
            for (const place of this.currentRegion.places) {
                ...
                // Trier les jobs par tier pour s'assurer que les postes importants sont pourvus en premier
        ...
        async _step4_simulateWorld() {
            ...
            // Snapshot pour la logique de surpopulation de step4.js
            ...
            for (let i = 0; i < totalTicks; i++) {
                ...
                if (simState.currentTick % 5 === 0) { this._handleMigration(dynastyMemberIds); }
        ...
        _formFamilies(individuals, place) {
            ...
            while (men.length > 0 && women.length > 0) {
                ...
                if (womanIndex !== -1) {
                    ...
                    const family = { id: `fam_${Date.now()}_${Math.random()}`, name: familyName, locationId: place.id, memberIds: [man.id, woman.id], isCustom: false };
        ...
        _handleRetirement(population, place) { population.forEach(person => { if (!person.isAlive || !person.job || person.status === 'Retraité(e)') return; const raceData = this.RACES_DATA.races[person.race]; if (!raceData) return; const jobData = this._getJobData(person.job.buildingName, person.job.jobTitle); if (!jobData || jobData.tier === 0) return; let retirementAge = Math.floor(raceData.esperanceVieMax * 0.90); if (person.age >= retirementAge) { person.job = null; person.status = 'Retraité(e)'; } }); }
                    ...
                    if (potentialWives.length > 0) {
                        ...
                        if (wife) {
                            ...
                            if (oldFamily) {
                                ...
                                if (oldFamily.memberIds.length === 0) { const placeOfOldFamily = this.currentRegion.places.find(p => p.id === oldFamily.locationId); if (placeOfOldFamily) placeOfOldFamily.demographics.families = placeOfOldFamily.demographics.families.filter(f => f.id !== oldFamily.id); }
                            }
        ...
        _handleMigration(dynastyMemberIds) { const allAvailableJobsByPlace = new Map(); this.currentRegion.places.forEach(p => allAvailableJobsByPlace.set(p.id, this._getAvailableJobs(p))); const allUnemployed = []; this.currentRegion.places.forEach(place => { const unemployed = place.demographics.population.filter(p => p.isAlive && !p.job && p.status === 'Actif' && p.age >= this.RACES_DATA.races[p.race].ageTravail && !dynastyMemberIds.has(p.id)); unemployed.forEach(p => allUnemployed.push({ person: p, sourcePlace: place })); }); if (allUnemployed.length === 0) return; const personToMoveData = allUnemployed[Math.floor(Math.random() * allUnemployed.length)]; const { person, sourcePlace } = personToMoveData; let bestOffer = { destination: null, job: null, score: 0 }; for (const destinationPlace of this.currentRegion.places) { if (destinationPlace.id === sourcePlace.id) continue; const availableJobs = allAvailableJobsByPlace.get(destinationPlace.id); if (availableJobs.length > 0) { const distance = this._axialDistance(sourcePlace.coords, destinationPlace.coords); const score = (destinationPlace.state.satisfaction / Math.max(distance, 1)) * Math.random(); if (score > bestOffer.score) { bestOffer = { destination: destinationPlace, job: availableJobs[0], score: score }; } } } if (bestOffer.destination) { const { destination, job } = bestOffer; const personIndex = sourcePlace.demographics.population.findIndex(p => p.id === person.id); if (personIndex > -1) sourcePlace.demographics.population.splice(personIndex, 1); person.locationId = destination.id; person.job = { locationId: destination.id, buildingName: job.buildingName, jobTitle: job.jobTitle }; destination.demographics.population.push(person); const family = sourcePlace.demographics.families.find(f => f.id === person.familyId); if (family) { const memberIndex = family.memberIds.indexOf(person.id); if (memberIndex > -1) family.memberIds.splice(memberIndex, 1); if (family.memberIds.length === 0) { const familyIndex = sourcePlace.demographics.families.findIndex(f => f.id === family.id); if (familyIndex > -1) sourcePlace.demographics.families.splice(familyIndex, 1); } } const newFamily = { id: `fam_mig_${Date.now()}_${Math.random()}`, name: person.lastName, locationId: destination.id, memberIds: [person.id], isCustom: false }; destination.demographics.families.push(newFamily); person.familyId = newFamily.id; } }
        _applyDynasticTitles(ruler, population) { const allPopulation = population; delete ruler.royalTitle; const spouse = this._getPersonById(ruler.spouseId, allPopulation); if (spouse?.isAlive) { spouse.job = null; spouse.royalTitle = 'Famille Gouvernante'; } const parents = this._getParents(ruler, allPopulation); parents.forEach(parent => { if (parent?.isAlive && !this._getJobData(parent.job?.buildingName, parent.job?.jobTitle)?.tier === 0) { parent.job = null; parent.royalTitle = parent.gender === 'Homme' ? 'Patriarche' : 'Matriarche'; const parentSpouse = this._getPersonById(parent.spouseId, allPopulation); if (parentSpouse?.isAlive && !parents.some(p => p.id === parentSpouse.id)) { parentSpouse.job = null; parentSpouse.royalTitle = 'Famille Gouvernante'; } } }); const children = this._getChildren(ruler, allPopulation); children.forEach(child => { if (child?.isAlive) { child.job = null; child.royalTitle = child.gender === 'Homme' ? 'Héritier' : 'Héritière'; const childSpouse = this._getPersonById(child.spouseId, allPopulation); if (childSpouse?.isAlive) { childSpouse.job = null; childSpouse.royalTitle = 'Famille Gouvernante'; } this._getChildren(child, allPopulation).forEach(grandChild => { if (grandChild?.isAlive) { grandChild.job = null; grandChild.royalTitle = 'Noble de la Cour'; const grandChildSpouse = this._getPersonById(grandChild.spouseId, allPopulation); if (grandChildSpouse?.isAlive) { grandChildSpouse.job = null; grandChildSpouse.royalTitle = 'Famille Gouvernante'; } } }); } }); const siblings = this._getSiblings(ruler, allPopulation); siblings.forEach(sibling => { if (sibling?.isAlive) { sibling.job = null; sibling.royalTitle = sibling.gender === 'Homme' ? 'Frère du pouvoir' : 'Sœur du pouvoir'; const siblingSpouse = this._getPersonById(sibling.spouseId, allPopulation); if (siblingSpouse?.isAlive) { siblingSpouse.job = null; siblingSpouse.royalTitle = 'Famille Gouvernante'; } this._getChildren(sibling, allPopulation).forEach(nephew => { if (nephew?.isAlive) { nephew.job = null; nephew.royalTitle = 'Noble de la Cour'; const nephewSpouse = this._getPersonById(nephew.spouseId, allPopulation); if (nephewSpouse?.isAlive) { nephewSpouse.job = null; nephewSpouse.royalTitle = 'Famille Gouvernante'; } this._getChildren(nephew, allPopulation).forEach(grandNephew => { if (grandNephew?.isAlive) { grandNephew.job = null; grandNephew.royalTitle = 'Noble de la Cour'; const grandNephewSpouse = this._getPersonById(grandNephew.spouseId, allPopulation); if (grandNephewSpouse?.isAlive) { grandNephewSpouse.job = null; grandNephewSpouse.royalTitle = 'Famille Gouvernante'; } } }); } }); } }); }
        _getDescendantMap(ruler, population) { const descendantMap = new Map(); const queue = [{ personId: ruler.id, generation: 0 }]; const visited = new Set(); descendantMap.set(ruler.id, 0); visited.add(ruler.id); const spouse = this._getPersonById(ruler.spouseId, population); if (spouse?.isAlive && !visited.has(spouse.id)) { descendantMap.set(spouse.id, 0); visited.add(spouse.id); } while (queue.length > 0) { const { personId, generation } = queue.shift(); const person = this._getPersonById(personId, population); if (person?.childrenIds) { person.childrenIds.forEach(childId => { if (!visited.has(childId)) { visited.add(childId); queue.push({ personId: childId, generation: generation + 1 }); } }); } } return descendantMap; }
        _getRulingDynastyMemberIds(rulers, population) { const dynastyIds = new Set(); rulers.forEach(ruler => { if (dynastyIds.has(ruler.id)) return; const descendants = this._getDescendantMap(ruler, population); descendants.forEach((gen, personId) => { dynastyIds.add(personId); const person = this._getPersonById(personId, population); if (person?.spouseId) dynastyIds.add(person.spouseId); }); this._getSiblings(ruler, population).forEach(sibling => { if (sibling) { dynastyIds.add(sibling.id); if (sibling.spouseId) dynastyIds.add(sibling.spouseId); this._getChildren(sibling, population).forEach(nephew => { if (nephew) { dynastyIds.add(nephew.id); if (nephew.spouseId) dynastyIds.add(nephew.spouseId); } }); } }); this._getParents(ruler, population).forEach(parent => { if (parent) { dynastyIds.add(parent.id); if (parent.spouseId) dynastyIds.add(parent.spouseId); } }); }); return dynastyIds; }
